...
const [featProgramIds, setFeatProgramIds] = useState(null)
  ...
  const getNameOfFeatured = () => {
    ...
    return programList[featProgramIds-1]['name']
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featProgramIds === null ? 'royalblue' : 'gold', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { setFeatProgramIds(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
